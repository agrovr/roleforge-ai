...
test("landing footer reads as a complete product footer", () => {
  ...
  assert.match(landingPage, /Secure Stripe billing/);
  assert.match(landingPage, /Account export available/);
  assert.match(landingPage, /Support request history/);
  ...
  assert.match(stylesheet, /\.footer-trust-row\s*\{(?=[^}]*display:\s*grid)(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*170px\),\s*1fr\)\))[^}]*\}/s);
  ...
  assert.match(stylesheet, /html\[data-theme="dark"\]\s+\.footer-trust-row span/);
});
