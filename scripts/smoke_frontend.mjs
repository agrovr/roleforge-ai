...
async function checkPublicShell(baseUrl) {
  ...
  requireCondition(/@media\s*\(min-width:\s*981px\)\s+and\s+\(max-width:\s*1440px\)\s*\{[\s\S]*?\.cta-visual\s*\{(?=[^}]*justify-self:\s*end)(?=[^}]*inline-size:\s*min\(100%,\s*320px\))(?=[^}]*min-block-size:\s*clamp\(292px,\s*26vw,\s*338px\))[^}]*\}[\s\S]*?\.cta-visual\s+\.resume-card\s*\{(?=[^}]*left:\s*52%)(?=[^}]*transform:\s*translateX\(-30%\)\s*rotate\(5deg\))[^}]*\}/s.test(stylesheetText), "landing final CTA front resume art is missing desktop-zoom containment");
