...
async function checkPublicShell(baseUrl) {
  ...
  requireCondition(
    /\.dash-mock\s+\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*(?:190|210|230)px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText) ||
      /\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*(?:190|210|230)px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText) ||
  ...
  requireCondition(/@media\s*\(min-width:\s*981px\)\s+and\s+\(max-width:\s*1440px\)\s*\{[\s\S]*?\.cta-visual\s+\.resume-card\s*\{(?=[^}]*left:\s*52%)(?=[^}]*transform:\s*translate(?:X)?\(-10%\)\s*rotate\(5deg\))[^}]*\}/s.test(stylesheetText), "landing final CTA front resume art is missing desktop-zoom containment");
