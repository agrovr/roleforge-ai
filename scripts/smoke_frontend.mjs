...
async function checkPublicShell(baseUrl) {
  ...
  requireCondition(/\.dash-mock\s+\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*(?:190|210|230)px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText), "landing dashboard stats can still squeeze cards at desktop widths");
