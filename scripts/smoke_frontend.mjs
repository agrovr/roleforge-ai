...
async function checkPublicShell(baseUrl) {
  ...
  requireCondition(/\.dash-stat-value\s*\{(?=[^}]*font-size:\s*clamp\(1\.58rem,\s*5\.35cqi,\s*2\.16rem\))(?=[^}]*overflow-wrap:\s*anywhere)(?=[^}]*text-wrap:\s*balance)(?=[^}]*white-space:\s*normal)[^}]*\}/s.test(stylesheetText), "landing dashboard stat values are not using overflow-safe fitted type");
  requireCondition(/@container\s+dash-stat\s*\(max-width:\s*250px\)\s*\{[^}]*\.dash-stat-value\s*\{[^}]*font-size:\s*clamp\(1\.45rem,\s*10\.5cqi,\s*1\.82rem\)/s.test(stylesheetText), "landing dashboard stat values are missing fitted type for compact cards");
  requireCondition(/@container\s+dash-stat\s*\(max-width:\s*210px\)\s*\{[^}]*\.dash-stat-value\s*\{[^}]*font-size:\s*clamp\(1\.28rem,\s*10cqi,\s*1\.56rem\)/s.test(stylesheetText), "landing dashboard stat values are missing fitted type for cramped cards");
  requireCondition(/@media\s*\(max-width:\s*1320px\)\s*\{[^}]*\.dash-stats\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/s.test(stylesheetText), "landing dashboard stats are missing viewport fallback columns");
