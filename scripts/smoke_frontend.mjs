...
async function checkPublicShell(baseUrl) {
  ...
  requireCondition(/\.dash-stat-value\s*\{(?=[^}]*font-size:\s*clamp\(1\.28rem,\s*9cqi,\s*1\.68rem\))(?=[^}]*line-height:\s*1\.06)(?=[^}]*word-break:\s*keep-all)[^}]*\}/s.test(stylesheetText), "landing dashboard stat values are missing the final fitted-type override");
