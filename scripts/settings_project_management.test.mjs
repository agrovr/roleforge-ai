...
test("settings saved project stage controls are compact and overflow-safe", () => {
  ...
  assert.match(stylesheet, /\.settings-project-summary\s*\{(?=[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\))(?=[^}]*align-items:\s*start)[^}]*\}/s);
  assert.match(stylesheet, /\.settings-project-controls\s*\{(?=[^}]*justify-self:\s*stretch)(?=[^}]*inline-size:\s*100%)(?=[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\))(?=[^}]*max-width:\s*100%)(?=[^}]*overflow:\s*hidden)(?=[^}]*border-top:\s*1px\s+solid)[^}]*\}/s);
