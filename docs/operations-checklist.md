...
Stripe test card numbers are test-mode only. Use the live promotion code generated by the proof runner for no-charge live checkout verification. The entitlement check waits for webhook delivery and only passes when the account is Premium with active or trialing billing.

## Support Operations

Support requests are always stored as signed-in, account-linked Supabase rows and appear in Support and Settings history. To also send an ops notification for each saved request, configure these Vercel Production variables:

```powershell
npx vercel env add ROLEFORGE_SUPPORT_WEBHOOK_URL production --scope team_kEe4HW272D5nYJDD92amj55H
...
After configuring the webhook, prove delivery without creating a customer ticket:
...
The smoke sends a clearly marked `support_request.test` payload with reference `RF-SMOKE`. It does not create a Supabase `support_requests` row and does not print the webhook secret.
