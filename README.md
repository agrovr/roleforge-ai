...
`npm run smoke:frontend` checks the live RoleForge shell, login copy, anonymous `/app`, settings, saved-project, download, and billing auth gates, crawler metadata, auth status, and backend `/capabilities` contract against `https://roleforgeai.vercel.app` and the production Cloud Run backend by default. Set `ROLEFORGE_SITE_URL` or `ROLEFORGE_BACKEND_URL` to target another deployment. Set `ROLEFORGE_SMOKE_COOKIE` only for a dedicated smoke account when you want it to also verify signed-in account status, the studio shell, saved-project API access, and settings plan details; do not use a personal browser cookie in CI. Set `ROLEFORGE_REQUIRE_SIGNED_IN_SMOKE=true` after the dedicated smoke cookie is configured so CI fails instead of silently skipping signed-in checks. If the smoke cookie belongs to a premium account, set `ROLEFORGE_EXPECT_PREMIUM_ACCESS=true` so the smoke fails when Premium or DOCX/TXT export access is missing.
